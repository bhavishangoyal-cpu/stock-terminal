...
def print_position_status(symbol, entry, current_price, peak_price, qty):
    pnl_per_share = current_price - entry
    pnl_total = pnl_per_share * qty
    pnl_percent = (pnl_per_share / entry) * 100 if entry > 0 else 0
    print(f"⏳ [{symbol}] Qty: {qty} shares | Entry: ${entry:.2f} | Now: ${current_price:.2f} | Peak: ${peak_price:.2f} | PnL: {pnl_percent:.2f}% (${pnl_total:.2f})")# ============================================================
# END PART 1
# ============================================================
# ============================================================
# MARKET DATA ENGINE
...
class PositionManager:
    ...
    def open_position(self, symbol, qty, entry, atr):
        self.symbol = symbol
        self.qty = qty
        ...
        while True:
            try:
                ...
                if not inside_window and self.position.active():
                    print("Outside entry window — no new buys, but still managing open position.")
                ...
                if self.position.active():
                    ...
                    if price is not None:
                        ...
                        if self.position.target_hit(price):
                            ...
                            self.logger.log(symbol, "SELL", self.position.qty, price, "2 ATR TARGET")
                            self.daily_realized_pnl += (price - self.position.entry_price) * self.position.qty
                        ...
                        if signal["sell"]:
                            ...
                            self.order.sell(data.contract, self.position.qty)
                            self.logger.log(symbol, "SELL", self.position.qty, price, "FINAL SELL")
                            self.daily_realized_pnl += (price - self.position.entry_price) * self.position.qty
                            self.daily_trade_count += 1
                            self.position.close_position()
                ...
                for symbol, data in self.market.items():
                    ...
                    if signal["buy"]:
                        ...
                        if trade:
                            ...
                            self.position.open_position(symbol, qty, fill_price, signal["atr"])
                            self.logger.log(symbol, "BUY", qty, fill_price, "FINAL BUY")
                        break
